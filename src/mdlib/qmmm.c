/*
 * $Id: qmmm.c,v 1.1.2.3 2008/02/29 07:02:52 spoel Exp $
 * 
...
void punch_QMMM_excl(t_QMrec *qm,t_MMrec *mm,t_atoms *atoms)
{
  ...
  for(i=0;i<qm->nrQMatoms;i++){
    nrexcl = 0;
    for(j=atoms->excl.index[qm->indexQM[i]];
	j<atoms->excl.index[qm->indexQM[i]+1];
	j++){
      for(k=0;k<mm->nrMMatoms;k++){
	if(mm->indexMM[k]==atoms->excl.a[j]){/* the excluded MM atom */
	  if(nrexcl >= max){
	    max += 1000;
	    srenew(excluded,max);
	  }	
		  ...
		  t_forcerec *fr)
{
  ...
  for(j=0;j<jmax;j++){
    ...
    if(qr->QMMMscheme==eQMMMschemeoniom){
       ...
       */
      /*      for (k=0;k<qm_nr;k++)
       *	md->bQM[qm_arr[k]]=TRUE; DONE IN mdatoms.c 
       */
      qr->qm[j] = mk_QMrec(); 
	...
	if(md->bQM[dummy] && md->bQM[aj] && md->bQM[ai]){
	...
	for(i=0;i<qm_nr;i++){
	  qr->qm[j]->c6[i]  =  C6(fr->nbfp,top->idef.atnr,
				 md->typeA[qm_arr[i]],
				  md->typeA[qm_arr[i]])/c6au;
	  qr->qm[j]->c12[i] = C12(fr->nbfp,top->idef.atnr,
				 md->typeA[qm_arr[i]],
				  md->typeA[qm_arr[i]])/c12au;
	}
      }
	...
	else if((md->bQM[ai]==FALSE) && md->bQM[aj]){
  ...
  if(qr->QMMMscheme!=eQMMMschemeoniom){
     ...
     */
    for (k=0;k<qm_nr;k++){
      /*      md->bQM[qm_arr[k]]=TRUE; done in mdatoms from v. 3.3 onward */
		    ...
		    t_topology *top)
{
  ...
  if(qr->QMMMscheme==eQMMMschemenormal){ /* also implies 1 QM-layer */
	...
	mm->c12[i] =C12(fr->nbfp,top->idef.atnr,
      ...
      punch_QMMM_excl(qr->qm[0],mm,&(top->atoms));
    }
