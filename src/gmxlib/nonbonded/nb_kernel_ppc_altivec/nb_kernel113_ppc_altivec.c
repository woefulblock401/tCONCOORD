 ...
 *
 * $Id: nb_kernel113_ppc_altivec.c,v 1.1 2004/12/26 19:26:00 lindahl Exp $
 * 
...
void 
nb_kernel113_ppc_altivec  (int *             p_nri,
                       int               iinr[],
                       int               jindex[],
                       int               jjnr[],
                       int               shift[],
					   ...
					   float *           work)
{
	...
	vector float rinvH1,rinvH2,rinvM,rinvsqO;
		...
		*count           = nn1;
		...
		for(n=0;n<nri;n++) {
			...
			for(k=nj0; k<(nj1-3); k+=4) 
            {
				...
				Vvdw12          = vec_madd(c12,vec_madd(rinvsix,rinvsix,nul),nul);
            ...
            else if(k<(nj1-1))
            {
				...
				rinvsqO         = do_recip(rsqO);
				do_3_invsqrt(rsqM,rsqH1,rsqH2,&rinvM,&rinvH1,&rinvH2);
				zero_highest_2_elements_in_vector(&rinvsqO);
            ...
            else if(k<nj1) 
            {
				...
				rinvsqO         = do_recip(rsqO);
				do_3_invsqrt(rsqM,rsqH1,rsqH2,&rinvM,&rinvH1,&rinvH2);
				zero_highest_3_elements_in_vector(&rinvsqO);
...
void 
nb_kernel113nf_ppc_altivec(int *             p_nri,
                       int               iinr[],
                       int               jindex[],
                       int               jjnr[],
                       int               shift[],
					   ...
					   float *           work)
{
	...
	vector float vctot,qqM,qqH,iqM,iqH,jq;
	vector float rinvsqO,rinvH1,rinvH2,rinvM,rsqO,rsqH1,rsqH2,rsqM;  
